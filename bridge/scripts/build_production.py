...
for i in range(18):
    ...
    f.keyframe_insert(data_path='location',frame=10500+i*5); f.location.z+=.35; f.keyframe_insert(data_path='location',frame=10800+i*5)

cam=bpy.data.objects.get('Cinematic Camera') or bpy.data.objects.get('Camera'); scene.camera=cam; cam.data.dof.use_dof=True; cam.data.dof.aperture_fstop=3.0
poses=[(0,(9,-15,6.5),(0,0,2.1),52),(25,(4,-12,4.5),(0,0,2.3),58),(55,(-4,-10,3.7),(-.5,0,2.4),62),(75,(5,-11,4.6),(0,-.5,2.4),55),(115,(1,-8,3.5),(0,-.8,1.2),70),(145,(7,-12,4.8),(0,0,2.3),52),(185,(3,-9,4.0),(1,-1,1.9),60),(220,(-5,-10,4.4),(0,-.5,2.1),56),(260,(2,-7,3.4),(0,-.8,1.0),72),(300,(-7,-12,5.0),(-1,0,2.1),50),(335,(7,-12,5.0),(1,-1,2.0),55),(375,(1,-13,4.2),(0,0,2.2),65),(415,(-8,-10,5.3),(0,0,2.4),52),(455,(7,-12,5.0),(0,0,2.3),55),(490,(1,-11,3.7),(0,-.5,1.8),68),(520,(-6,-10,5.2),(0,0,2.2),54),(550,(5,-13,5.6),(0,-.5,2.3),58),(580,(0,-14,4.8),(0,-.5,2.2),55),(600,(0,-15,4.8),(0,-.5,2.2),55)]
...
for sec,x in [(0,-1),(75,-.2),(145,1),(220,0),(300,-2),(375,1.5),(455,0),(520,-.5),(580,0)]:
    f=int(sec*FPS)
    for o in milo_parts:
        if o.parent is None:o.location.x += x;o.keyframe_insert(data_path='location',index=0,frame=f)
...
for f,z in [(1,.50),(7200,.50),(10500,.52),(11200,1.0),(11800,1.55),(12400,1.9),(END,1.9)]: seed.location.z=z; seed.keyframe_insert(data_path='location',index=2,frame=f)
for f,s in [(1,.03),(11200,.03),(11800,.35),(12400,1.0),(END,1.0)]: flower.scale=(s,s,s); flower.keyframe_insert(data_path='scale',frame=f)
for l in [o for o in scene.objects if o.type=='LIGHT']:
    if l.data.type=='SUN': l.data.energy=1.8; l.data.keyframe_insert(data_path='energy',frame=1); l.data.energy=1.0; l.data.keyframe_insert(data_path='energy',frame=END)
...
seq=master.sequence_editor_create(); ss=seq.strips.new_scene('EPISODE 3D',scene,1,1); ss.frame_final_duration=END
ms=seq.strips.new_sound('Original Music',music,2,1); ms.frame_final_duration=END; ms.volume=.45
for i,(sec,role,text) in enumerate(lines):
    p=os.path.join(AUDIO,'line_%02d.wav'%i)
    if os.path.exists(p):
        s=seq.strips.new_sound(role+'_'+str(i),p,10+i,1+int(sec*FPS)); s.volume=1.0
